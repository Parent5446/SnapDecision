...
function urandom( $len ) {
	...
	if ( $remaining > 0 && function_exists( 'openssl_random_pseudo_bytes' ) && strncasecmp( PHP_OS, 'WIN', 3 ) !== 0
	) {
	...
	// Try /dev/urandom
	if ( $remaining > 0 && is_readable( '/dev/urandom' ) && ( $rand = fopen( '/dev/urandom', 'rb' ) )
	) {
		$chunkSize = 1024 * 8;
